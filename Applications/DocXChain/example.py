...
from modules.file_loading import load_document, load_whole_pdf_shin
...
from pipelines.pdf2html import Document2Html
...
def document_structurization_example(image):
    ...
    layout_analysis_configs['model_path'] = './weights/DocXLayout_231012.pth'  # note that: currently the layout analysis model is NOT from modelscope
    ...
    formula_recognition_configs = dict()
    formula_recognition_configs['from_modelscope_flag'] = False
    formula_recognition_configs['image_resizer_path'] = './weights/LaTeX-OCR_image_resizer.onnx'
    formula_recognition_configs['encoder_path'] = './weightsLaTeX-OCR_encoder.onnx'
    formula_recognition_configs['decoder_path'] = './weights/LaTeX-OCR_decoder.onnx'
    formula_recognition_configs['tokenizer_json'] = './weights/LaTeX-OCR_tokenizer.json'
    configs['formula_recognition_configs'] = formula_recognition_configs
...
def pdf2html(image_list, pdf_path):
    ...
    formula_recognition_configs['from_modelscope_flag'] = False
    formula_recognition_configs['image_resizer_path'] = './weights/LaTeX-OCR_image_resizer.onnx'
    formula_recognition_configs['encoder_path'] = './weights/LaTeX-OCR_encoder.onnx'
    formula_recognition_configs['decoder_path'] = './weights/LaTeX-OCR_decoder.onnx'
    formula_recognition_configs['tokenizer_json'] = './weights/LaTeX-OCR_tokenizer.json'
    configs['formula_recognition_configs'] = formula_recognition_configs

    # initialize
    document_structurizer = Document2Html(configs)
...
def main():
    ...
    parser.add_argument("task", choices = ['general_text_reading', 'table_parsing', 'formula_recognition', 'document_structurization', 'whole_pdf_conversion', 'pdf2html'], help = "specify the task to be performed", type = str)
    ...
    elif args.task == 'pdf2html':
        image_list = load_whole_pdf_shin(args.document_path)
    else:
    ...
    if image is not None or image_list is not None:
        if args.task == 'general_text_reading':
            ...
            final_result, output_image = pdf2html(image_list, args.document_path)
